...
export function getTableHeaders(tableHeaders: TableHeaderType[]) {
  ...
  const headers = tableHeaders?.map((header: TableHeaderType) => {
    return <Table.HeadCell key={ generateUniqKey("table-header") } className={ `w-[${ header.width ? header.width : "280px" }] bg-base-gray-800 text-white` }>
...
export function getTableBody(data: any[], url: string, headers: (string | undefined)[]) {
  ...
  const tableBody = data.map((element, idx) =>
    <Table.Row key={ generateUniqKey("table-body-row") } className="cursor-pointer border-teal-800/30">
      <Table.Cell onClick={ () => { if(url) goToDetails(url, element._id) } }>
        { idx + 1 }
      </Table.Cell>
      {Object.entries(element).filter(([key]) => headers.includes(key)).map((hEl) => {
        return <Table.Cell key={ generateUniqKey("table-cell") } className="max-w-[280px]" onClick={ () => { if(url) goToDetails(url, element._id) } }>
          { hEl[1] as string }
        </Table.Cell>;
      })}
      { headers.includes("actions") ? (
      <Table.Cell className="w-44">
        <ActionButtons url={ `${ url }/${ element._id }` }/>
      </Table.Cell>
      ): null }
    </Table.Row>
  )
