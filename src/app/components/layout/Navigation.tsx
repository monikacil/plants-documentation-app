          ...
          <section className='hidden w-full md:block md:w-auto'>
            <ul className='flex flex-col items-center font-medium p-4 md:p-0 md:flex-row gap-5'>
              {navLinks.map((el: LinkList) => {
                if (el.type === "dropdown") {
                  return (
                    <li key={generateUniqKey(`dropdown-${el.name}`)} className='relative'>
                      <Dropdown
                        title={el.name}
                        className='hover:text-gray-green-700'
                        icon={
                          el.icon ? (
                            <FaCircleUser size={40} className='hover:text-gray-green-700' />
                          ) : undefined
                        }
                      >
                          ...
                          {el.options?.map((link: Link) => {
                            if (link.type === "button" && link.action) {
                              return (
                                <li
                                  ...
                                  className={`hover:bg-gray-100 rounded-lg ${
                                    link.href === pathname ? "bg-gray-green-200" : ""
                                  }`}
                                >
                                  <button
                                    onClick={link.action}
                              ...
                              return (
                                ...
                                >
                                  <Link href={link.href} scroll={false} className='block px-4 py-2'>
                                    {link.name}
                                  </Link>
                                </li>
                              );
                  ...
                  return (
                    <li
                      ...
                      className={`hover:text-gray-green-700 rounded-lg ${
                        el.href === pathname ? "bg-gray-green-200" : ""
                      }`}
                    >
                      <Link
